...
namespace Soho.EmailAndSMS.Send.SMS.Sender
{
    /// <summary>
    /// 亿美接口发送方式
    /// </summary>
    public class EmaySMSSender : ISenderInterface
    {
        /// <summary>
        /// 亿美接口发送方式
        /// </summary>
        ...
        #region 异步使用亿美短信接口发送短信
        public delegate string EmaySendSMSDelegate(Dictionary<string, string> serivceConfig, SMSEntity sms);
        ...
        {
            EmaySendSMSDelegate _emaySendSMS = ar.AsyncState as EmaySendSMSDelegate;
            string sendResult = _emaySendSMS.EndInvoke(ar);

            int splitIndex = sendResult.IndexOf("#SPLIT#");
            long sysNo = long.Parse(sendResult.Substring(0, sendResult.IndexOf("#SPLIT#")));
            sendResult = sendResult.Substring(splitIndex + 7);

            //更新电子邮件发送结果
            SMSStatus status = sendResult.Equals("0") ? SMSStatus.SendSuccess : SMSStatus.SendFailure;
            string note = string.Format("{0}：{1}；", DateTime.Now, sendResult.Equals("0") ? "OK" : sendResult);
            SMSProcessor.Instance.UpdateSMSStatus(sysNo, status, note);
        }
        #endregion
    }
